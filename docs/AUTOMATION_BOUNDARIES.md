...
| **E Experience** | Detectors/scorers may submit non-authoritative proposals; M may manage decay, hits, activation, and reviewed lifecycle after creation. | Primary-agent write path with `e_authored_by` and `e_initial_priority`; optional proposal queue. | Authoring E content, assigning its initial order, backfilling missing E, or overwriting the primary agent's words. |
