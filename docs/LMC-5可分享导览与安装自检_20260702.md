...
| E | Experience 体验信号 | 主 AI 如何记住它，初始顺序是什么？ | `e_authored_by`、`e_initial_priority`、体验坐标 |
...
E 轴的正式内容必须由正在与用户交互的主 AI 亲自写下，并由主 AI
设定 `e_initial_priority` 作为初始顺序。housekeeper/scorer 只能提交待审
proposal，不得首写、补写或代替主 AI 排序。初始顺序落库后，再交给 M 轴
根据时间、命中次数、激活和审核过的生命周期规则管理。
...
| `e_axis_scorer.py` | E 轴非权威 proposal 建议器 |
| `e_axis_trigger.py` | proposal 触发门，禁止直写 E |
...
| E ranking gate | 代表性样本验证达标后再让 E 参与排序，无固定等待期 |
...
这只说明空 runner 能启动。真正部署必须有自己的入口把 `consolidate`、`hippocampus`、`z_audit`、`patrol` 等 callable 注入 `DreamRunner`。E 轴不允许自动 backfill。
...
| E resonance | 候选有 `e_authored_by`、`e_initial_priority`、`valence/arousal`，category 在 eligible set 内 | emotion 通道永远空或混入越权记录 |
...
| E 轴让人格飘 | housekeeper/scorer 越权首写或首排 | 主 AI 亲写并设初始优先级，自动化只管理后续代谢 |
...
- E 记录均有 `e_authored_by` 和 `e_initial_priority`；scorer 只写 proposal queue。
- M patrol 只读。
...
├─ Persona：identity protected、Z manual、主 AI 亲写 E 并首排、half-life inf、spontaneous schedule
