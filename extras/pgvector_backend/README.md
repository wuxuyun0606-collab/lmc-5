...
| `dream_runner.py` | new — the nightly pipeline orchestrator | Chains the full dream pass into one cron-able entry: consolidate → hippocampus → heartbeat_detect → e_axis_backfill → narrative_weekly → narrative_monthly (first 3 days of month) → z_audit → patrol. Each step is an injected callable; `None` = skip. Failure-isolated — one step crashing doesn't block the rest. CLI: `python -m extras.pgvector_backend.dream_runner [--dry-run]`. |
| `heartbeat_detector.py` | new — the missing "what to save" layer for persona | Detects heartbeat moments (intimacy / physical reactions / nickname shifts) and emotional fragments (breakdown / crying / late-night emo / self-denial) from conversation chunks. Keyword gate (CN+EN bilingual) + optional LLM confirmation. Outputs `HeartbeatCandidate` / `EmotionCandidate` with `protected=True` — feeds into hippocampus as an extra candidate source. |
| `heartbeat_trigger.py` | new — real-time heartbeat detection for hooks | Per-message keyword gate that fires on intimacy / emotional triggers during live conversation. Injects a prompt into additionalContext telling the AI "this moment might be worth saving". Optional scene_lookup surfaces the last similar heartbeat. The AI decides whether to save — trigger ≠ auto-store. |
